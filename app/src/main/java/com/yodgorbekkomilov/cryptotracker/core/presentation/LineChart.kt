package com.yodgorbekkomilov.cryptotracker.core.presentation
...
import com.yodgorbekkomilov.cryptotracker.crypto.presentation.coin_detail.ChartStyle
import com.yodgorbekkomilov.cryptotracker.crypto.presentation.coin_detail.DataPoint
import com.yodgorbekkomilov.cryptotracker.crypto.presentation.coin_detail.ValueLabel
...
fun LineChart(
    ...
    Canvas(
        ...
        val maxYLabelWidth = yLabelTextLayoutResults.maxOfOrNull { it.size.height } ?: 0
        ...
        drawPoints = visibleDataPointsIndices.map {
            ...
            DataPoint(
                x = x,
                y = y,
                drawPoints.getOrNull(it)?.xLabel ?: " "

            )

        }

        val coinPoints1 = mutableListOf<DataPoint>()
        val coinPoints2 = mutableListOf<DataPoint>()
        for (i in 1 until drawPoints.size) {
            ...
            val y1 = p0.y
            val y2 = p1.y
            coinPoints1.add(DataPoint(x, y1, ""))
            coinPoints2.add(DataPoint(x, y2, ""))
        ...
        val linePath = androidx.compose.ui.graphics.Path().apply {
            if(drawPoints.isNotEmpty()) {
                ...
                for(i in 1 until drawPoints.size) {
                    cubicTo(
                        x1 = coinPoints1[i - 1].x,
                        y1 = coinPoints1[i - 1].y,
                        x2 = coinPoints2[i - 1].x,
                        y2 = coinPoints2[i - 1].y,
                        x3 = drawPoints[i].x,
                        y3 = drawPoints[i].y
                    )
                }
...
@Preview(apiLevel = 34 , widthDp = 1000)
@Composable
fun LineChartPreview(modifier: Modifier = Modifier) {
    CryptoTrackerTheme {
        ...
        val dataPoints = remember {
            coinHistoryRandomized.map {
                DataPoint(
                    ...
                    xLabel = DateTimeFormatter.ofPattern(
                        "ha\nM/d"
                    )
                        .format(it.dateTime)
        ...
        LineChart(
            dataPoints = dataPoints, style = style, visibleDataPointsIndices = 0..19, unit = "$",
            modifier = Modifier
                .width(
                    700.dp
                )
